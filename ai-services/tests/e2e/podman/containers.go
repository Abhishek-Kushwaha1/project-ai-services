...
var restartRe = regexp.MustCompile(`(?i)restarts?:\s*(?P<count>\d+)`)
...
//Restart Count Logic (minimal logs)
...
func getRestartCount(podName string) (int, error) {
        ...
        // RestartPolicy "no" , no restarts expected
...
// VerifyContainers checks container status and health
// Returns an error if any pod is unhealthy or has restarts; otherwise returns nil.
func VerifyContainers(appName string) error {
        ...
        if strings.TrimSpace(res) == "" {
                Skip("No pods found — skipping pod health validation")
                return nil
        }
        ...
        for _, row := range rows {
                ok := strings.HasPrefix(row.Status, "Running (healthy)") ||
                        row.Status == "Created"

                if !ok {
                        return fmt.Errorf("pod %s is not healthy (status=%s)",
                                row.PodName, row.Status)
                }
        }
        ...
        for _, suffix := range expectedPodSuffixes {
                ...
                Expect(actualPods).To(HaveKey(expectedPodName),
                        "expected pod %s to exist", expectedPodName)

                restartCount, err := getRestartCount(expectedPodName)
                Expect(err).NotTo(HaveOccurred())

                GinkgoWriter.Printf(
                        "[RestartCount] pod=%s restarts=%d\n",
                        expectedPodName, restartCount,
                )

                Expect(restartCount).To(BeNumerically("<=", 0),
                        fmt.Sprintf("pod %s restarted %d times",
                                expectedPodName, restartCount))
        }
